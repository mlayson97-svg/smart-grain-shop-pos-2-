    ...
    <header className="bg-slate-900 text-white border-b border-slate-800 sticky top-0 z-30 shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {localStorage.getItem('kaberege_pos_mode') === 'offline' ? (
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border text-[11px] font-bold bg-amber-500/15 text-amber-400 border-amber-500/30 shadow-xs">
              <Layers className="w-3.5 h-3.5 text-amber-400" />
              <span>{currentLanguage === 'sw' ? 'Hifadhi ya Ndani (Offline)' : 'Local Storage (Offline)'}</span>
            </div>
          ) : (
