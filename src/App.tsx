...
export default function App() {
  ...
  const [isOfflineMode, setIsOfflineMode] = useState<boolean>(localStorage.getItem('kaberege_pos_mode') === 'offline');
      ...
      if (firebaseUser) {
        try {
          ...
          if (profile && profile.status === 'active') {
            ...
            // Profile is inactive or deleted, sign out
            await signOut(auth);
            setCurrentUser(null);
          }
        } catch (err) {
        ...
        <div className="flex flex-col items-center space-y-4 max-w-md text-center">
          ...
          <div className="pt-6">
            <button
              ...
              className="px-4 py-2 text-xs font-bold text-amber-400 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 rounded-xl transition-all cursor-pointer shadow-sm uppercase tracking-wider"
            >
              {currentLanguage === 'sw' 
                ? 'Fungua Nje ya Mtandao (Offline Bypass)' 
                : 'Open Offline (Offline Bypass)'}
            </button>
            <p className="text-[10px] text-slate-500 mt-2 max-w-xs mx-auto leading-relaxed">
              {currentLanguage === 'sw'
                ? 'Bofya hapa kama muunganisho wa mtandao unachelewa kufungua mfumo haraka ukitumia kumbukumbu ya kivinjari chako tu.'
                : 'Click here if your network is slow or hanging to instantly launch using your browser cache only.'}
            </p>
          </div>
        </div>
      </div>
                  ...
                  <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-3">
                    ...
                    <div className="space-y-1.5 max-h-48 overflow-y-auto">
                      {purchases.slice(0, 5).map((p) => (
                        <div key={p.id} className="p-2 border rounded bg-slate-50/50 flex justify-between font-medium">
                          <div>
