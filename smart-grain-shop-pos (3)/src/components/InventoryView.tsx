          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-slate-100">
                  {stockMovements.slice(0, 100).map((m, idx) => {
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">
                    {t.category} {currentLanguage === 'sw' ? '(Si lazima)' : '(Optional)'}
                  </label>
                  <select
                  ...
                  >
                    <option value="">{currentLanguage === 'sw' ? '-- Chagua Kategoria (Si lazima) --' : '-- Choose Category (Optional) --'}</option>
                    {categories.map((cat, idx) => (
                      <option key={`${cat.id}-${idx}`} value={cat.id}>
                        {currentLanguage === 'sw' ? cat.nameSw : cat.nameEn}
                      </option>
