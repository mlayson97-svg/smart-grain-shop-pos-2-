...
export default function LoginView({
  ...
  useEffect(() => {
    async function checkFirstSetup() {
      ...
      // If we already have cached products or cached users, we know setup is complete. Avoid slow Firestore call on boot!
      const hasLocalProducts = localStorage.getItem('kaberege_products');
      const hasCachedUsers = localStorage.getItem('kaberege_cached_users');
      if (hasLocalProducts || hasCachedUsers) {
        setIsDbEmpty(false);
        setChecking(false);
        return;
      }
            ...
            <div className="space-y-4 text-xs text-slate-300">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  >
                    {cachedUsers.map((u, idx) => (
                      <option key={`${u.id}-${idx}`} value={u.id} className="bg-slate-950 text-white">
